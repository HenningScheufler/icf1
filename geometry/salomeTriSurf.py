...
class triSurf:
    def __init__(self, object = None, allEdges = False):
        ...
        for name, ids in patches.items():
            for faceId in ids:
                if patchIds[faceId-1] == lastPatchId:
                    ...
                    print("Face %d is assigned to both groups %s and %s" % (faceId, name, list(patches.keys())[patchIds[faceId-1]]))
        ...
        faceIds, patchIds = list(zip(*faceAndpatchIds))
        ...
        if not allEdges:
            edgeIds = []
            for name, ids in featureEdgeSubsets.items():
                edgeIds += ids
    ...
    def _writePatchDefs(self, f, typeName = 'wall'):
        ...
        typeWall = re.compile('W_*')
        typePatch = re.compile('P_*')
        
        f.write('%d\n(\n' % len(patches))
        for name in patches.keys():
            if typeWall.match(name):
                f.write('%s\t%s\n' % (name[2:], typeName))
            elif typePatch.match(name):
                typeName = 'patch'
                f.write('%s\t%s\n' % (name[2:], typeName))
            else:
                f.write('%s\t%s\n' % (name, typeName))
    ...
    def _writeFacets(self, f):
        ...
        f.write('%d\n(\n' % sum([len(patch) for patch in patches.values()]))
        
        patchId = 0
        for patchId, (patchName, faceIds) in enumerate(patches.items()):
